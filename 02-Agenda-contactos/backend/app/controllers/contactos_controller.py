...
# Nueva función para buscar cantoactos por categoria
def obtener_contactos_por_categoria(categoria: str) -> list[Contacto]:
    categorias_validas = ["personal", "trabajo", "familia"]
    categoria_lower = categoria.lower()
    
    # Validación
    if categoria_lower not in categorias_validas:        raise HTTPException(
            status_code=400, 
            detail=f"Categoría inválida. Los valores permitidos son: {', '.join(categorias_validas)}"
        )
    
    contactos = []
    for contacto in contactos_db:
        if contacto.categoria.lower() == categoria_lower:
